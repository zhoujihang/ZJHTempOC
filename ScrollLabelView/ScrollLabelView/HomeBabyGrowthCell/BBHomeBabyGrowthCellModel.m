...
@implementation BBHomeBabyGrowthCellModel
...
+ (NSArray *)fakeModelArray{
    ...
    NSArray *voteModelArray = @[[BBRemindCellVoteViewModel fakeModelWait], [BBRemindCellVoteViewModel fakeModelHidden], [BBRemindCellVoteViewModel fakeModelVoting], [BBRemindCellVoteViewModel fakeModelSelectOne], [BBRemindCellVoteViewModel fakeModelSelectTwo]];
    
    NSMutableArray *marr = [@[] mutableCopy];
    for (int i=0; i<10; i++) {
        BBHomeBabyGrowthCellModel *model = [[BBHomeBabyGrowthCellModel alloc] init];
        ...
        model.m_BabyWeight = [NSString stringWithFormat:@"%0.1f-%0.1fKG",2.3+i, 4.8+i];
        model.m_BabyHeight = [NSString stringWithFormat:@"%0.1f-%0.1fCM",45.3+i, 56.8+i];
        
        if (i==6) {
            model.m_VoteViewModel = nil;
        }else{
            int j = i%voteModelArray.count;
            model.m_VoteViewModel = voteModelArray[j];
        }
...
- (CGFloat)m_ViewHeight{
    ...
    // 高度适配器
    static BBHomeBabyGrowthCell *cell;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        cell = [[BBHomeBabyGrowthCell alloc] init];
    });
